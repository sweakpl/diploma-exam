...
class RequestQuestionsRedrawTest {

    private lateinit var allowQuestionsRedraw: AllowQuestionsRedraw
    ...
    fun setUp() {
        ...
        allowQuestionsRedraw = AllowQuestionsRedraw(questionsDrawRepository)
    }
    ...
    fun `If request is successful, then first value is Loading, second is Success`() = runTest {
        ...
        val flowValues = allowQuestionsRedraw.invoke().toList()
    ...
    fun `If request is successful, then emits exactly two values`() = runTest {
        ...
        Assert.assertEquals(
            2,
            allowQuestionsRedraw.invoke().count()
        )
    }
